...
class BatteryTestPublisher(Node):
    def __init__(self):
        ...
        self.voltage_pub = self.create_publisher(Float32, '/battery/pack_voltage', 10)
        self.current_pub = self.create_publisher(Float32, '/battery/current', 10)
        self.remaining_capacity_pub = self.create_publisher(Float32, '/battery/remaining_capacity', 10)
        self.total_capacity_pub = self.create_publisher(Float32, '/battery/total_capacity', 10)
        self.cycles_pub = self.create_publisher(Float32, '/battery/cycles', 10)
        self.temperature_pub = self.create_publisher(Float32MultiArray, '/battery/temperature', 10)
        ...
        self.cell_voltages_pub = self.create_publisher(Float32MultiArray, '/battery/cell_voltages', 10)
    ...
    def publish_battery_data(self):
        ...
        # Publish all data
        soc_msg = Float32()
        soc_msg.data = float(soc)
        self.soc_pub.publish(soc_msg)
        
        voltage_msg = Float32()
        voltage_msg.data = float(pack_voltage)
        ...
        remaining_msg.data = float(remaining_capacity)
        self.remaining_capacity_pub.publish(remaining_msg)
        
        total_msg = Float32()
        total_msg.data = float(self.total_capacity)
        ...
        temp_msg.data = [float(temp1), float(temp2)]
        ...
        cell_msg.data = [float(v) for v in cell_voltages]
